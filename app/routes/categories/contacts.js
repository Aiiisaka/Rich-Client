...
import MyContacts from 'projet-4/classes/contacts';
import { action } from '@ember/object';

export default class CategoriesContactsRoute extends Route {
  model(params) {
    return new MyContacts(this.store.findAll('contact')).filterBy(
      'category',
      params.category_id
    );
  }
  ...
  @action update(category, contact) {
    this.transitionTo(
      'categories.contacts.update',
      category.filterBy('contact_id', contact.id)
    );
  }
